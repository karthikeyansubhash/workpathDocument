...
class RequestPrintTask(context: TestJobFragment, private val mPrintAttributes: PrintAttributes) {
    ...
    suspend fun execute() {
        ...
        try {
            mContextRef.get()?.run {
                rid = PrinterService.submit(requireContext(), mPrintAttributes, null)
        ...
        withContext(Dispatchers.Main) {
            mContextRef.get()?.run {
                if (!TextUtils.isEmpty(rid)) {
                    ...
                    Logger.showResult(activity, getString(R.string.job_request_failed) + mThrowable)
                } else {
