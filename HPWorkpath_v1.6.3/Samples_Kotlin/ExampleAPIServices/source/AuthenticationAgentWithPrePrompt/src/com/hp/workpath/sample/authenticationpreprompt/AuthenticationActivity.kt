...
import com.hp.workpath.sample.authenticationpreprompt.databinding.ActivitySignInBinding
import com.hp.workpath.sample.authenticationpreprompt.databinding.LayoutManualZoneBinding
import com.hp.workpath.sample.authenticationpreprompt.task.InitializationTask
import com.hp.workpath.sample.authenticationpreprompt.task.InitializationTask.InitializeInterface
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.launch

class AuthenticationActivity : AppCompatActivity() {
    var isInitializedSDK = false
    ...
    private fun setVersion() {
        if (!SCREEN_4_3_INCH.equals(mBindingActivitySignIn.container.tag)) {
            try {
                ...
                Logger.showResult(this, "Workpath.getInstance ${t.message}")
            }
        }
    }
        ...
        try {
            if (v === mBindingLayoutManualZone.runButton) {
                if (TextUtils.isEmpty(mBindingLayoutManualZone.resultEditText.text.toString())) {
                    ...
                    val data = mBindingLayoutManualZone.resultEditText.text.toString()
                    val gson = Gson()
                ...
                mBindingLayoutManualZone.resultEditText.setText(gson.toJson(windowsData))
            }
        } catch (t: Throwable) {
    ...
    private var buttonClickListener = View.OnClickListener { v ->
        ...
        try {
            val result = Result()
            if (v === mBindingActivitySignIn.signInButton) {
                val signInAction = SignInAction(SignInAction.Action.SUCCESS, null)
                AccessService.signIn(this@AuthenticationActivity, signInAction, windowsData, result)
            } else if (v === mBindingActivitySignIn.failButton) {
                val failureMessage =
                    "This is failure message from AuthenticationAgentPrePrompt Sample."
            ...
            } else if (v === mBindingActivitySignIn.homeButton) {
                val signInAction = SignInAction(SignInAction.Action.HOME, null)
                AccessService.signIn(this@AuthenticationActivity, signInAction, null, result)
            }else if (v === mBindingActivitySignIn.backButton) {
    ...
    companion object {
        const val TAG = "[SAMPLE]" + "AuthAgentPre"
    }
}