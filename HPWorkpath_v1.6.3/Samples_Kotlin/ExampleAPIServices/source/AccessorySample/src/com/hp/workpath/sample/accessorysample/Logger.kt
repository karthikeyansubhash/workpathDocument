...
object Logger {
    ...
    fun build(hidReport: HIDReport?): String? {
        if (hidReport != null) {
            val logBuilder = StringBuilder()
            logBuilder.append(_START)
            logBuilder.append("Type").append(_EQ).append(hidReport.type).append(_C)
            logBuilder.append("Data").append(_EQ).append(Arrays.toString(hidReport.data))
            logBuilder.append(_END)
            return logBuilder.toString()
        }
        return null
