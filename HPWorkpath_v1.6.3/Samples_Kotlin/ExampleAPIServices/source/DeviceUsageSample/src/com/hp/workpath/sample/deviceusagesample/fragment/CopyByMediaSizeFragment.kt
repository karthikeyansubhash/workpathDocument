...
class CopyByMediaSizeFragment : Fragment() {

    private lateinit var mListAdapter: CopyByMediaSizeAdapter

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view = inflater.inflate(R.layout.fragment_copy_by_media_size, container, false)
        val mListView: RecyclerView = view.findViewById(R.id.printByMediaSizeView)
        mListAdapter = CopyByMediaSizeAdapter()
    ...
    private fun showJobCategoryAndMediaSize(data: String?) {
        val gson = Gson()
        val listType = object : TypeToken<List<PrinterInfo.CopyByMediaSize?>?>() {}.type
        val list = gson.fromJson<List<PrinterInfo.CopyByMediaSize>>(data, listType)
