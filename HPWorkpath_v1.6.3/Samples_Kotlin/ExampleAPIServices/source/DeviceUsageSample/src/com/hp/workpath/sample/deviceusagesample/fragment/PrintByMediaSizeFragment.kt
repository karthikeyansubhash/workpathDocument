...
class PrintByMediaSizeFragment : Fragment() {

    private lateinit var mListAdapter: PrintByMediaSizeAdapter

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view = inflater.inflate(R.layout.fragment_print_by_media_size, container, false)
        val mListView: RecyclerView = view.findViewById(R.id.printByMediaSizeView)
        mListAdapter = PrintByMediaSizeAdapter()
    ...
    private fun showJobCategoryAndMediaSize(data: String?) {
        val gson = Gson()
        val listType = object : TypeToken<List<PrinterInfo.PrintByMediaSize?>?>() {}.type
        val list = gson.fromJson<List<PrinterInfo.PrintByMediaSize>>(data, listType)
